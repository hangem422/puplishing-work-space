...
const LOCAL_STORAGE_KEY = '@bskp_gtas';
...
const ERROR_MESSAGE_03 =
  '해운대구민카드 발급 대상이 아닙니다. 해운대구민카드는 주민등록상 주소지가 부산광역시 해운대구로 등록된 시민만 발급받을 수 있습니다.';
const ERROR_MESSAGE_04 =
  '입력하신 주민등록번호가 유효하지 않습니다. 주민등록번호를 확인 후 다시 시도해주세요.';

const TERM_OF_USE_TITLE = '약관동의';
const CERTIFICATION_TITLE = '해운대구민인증';
...
function getVcFromApi(rrn, lastChance) {
  ...
  return get({
    ...
    .then((res) => {
      ...
      // 발급 조건 미달 오류
      if (['E001', 'E002', 'E003', 'E006'].includes(res.data.message)) {
